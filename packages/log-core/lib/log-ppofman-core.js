import chalk from 'chalk'

function logHyeonsu () {
  console.log(chalk.red('❤ hyeonsu ❤'))
  console.log(chalk.yellow('❤ hyeonsu ❤'))
  console.log(chalk.green('❤ hyeonsu ❤'))
  console.log(chalk.blue('❤ hyeonsu ❤'))
  console.log(chalk.bgRed('❤ hyeonsu ❤'))
  console.log(chalk.bgYellow('❤ hyeonsu ❤'))
  console.log(chalk.bgGreen('❤ hyeonsu ❤'))
  console.log(chalk.bgBlue('❤ hyeonsu ❤'))
}

function logJune () {
  console.log(chalk.red('❤ june ❤'))
  console.log(chalk.yellow('❤ june ❤'))
  console.log(chalk.green('❤ june ❤'))
  console.log(chalk.blue('❤ june ❤'))
  console.log(chalk.bgRed('❤ june ❤'))
  console.log(chalk.bgYellow('❤ june ❤'))
  console.log(chalk.bgGreen('❤ june ❤'))
  console.log(chalk.bgBlue('❤ june ❤'))
}

function logChoonti () {
  console.log(chalk.red('❤ choonti ❤'))
  console.log(chalk.yellow('❤ choonti ❤'))
  console.log(chalk.green('❤ choonti ❤'))
  console.log(chalk.blue('❤ choonti ❤'))
  console.log(chalk.bgRed('❤ choonti ❤'))
  console.log(chalk.bgYellow('❤ choonti ❤'))
  console.log(chalk.bgGreen('❤ choonti ❤'))
  console.log(chalk.bgBlue('❤ choonti ❤'))
}

function logDante () {
  console.log(chalk.red('❤ dante ❤'))
  console.log(chalk.yellow('❤ dante ❤'))
  console.log(chalk.green('❤ dante ❤'))
  console.log(chalk.blue('❤ dante ❤'))
  console.log(chalk.bgRed('❤ dante ❤'))
  console.log(chalk.bgYellow('❤ dante ❤'))
  console.log(chalk.bgGreen('❤ dante ❤'))
  console.log(chalk.bgBlue('❤ dante ❤'))
}

function logAyaan () {
  console.log(chalk.red('❤ ayaan ❤'))
  console.log(chalk.yellow('❤ ayaan ❤'))
  console.log(chalk.green('❤ ayaan ❤'))
  console.log(chalk.blue('❤ ayaan ❤'))
  console.log(chalk.bgRed('❤ ayaan ❤'))
  console.log(chalk.bgYellow('❤ ayaan ❤'))
  console.log(chalk.bgGreen('❤ ayaan ❤'))
  console.log(chalk.bgBlue('❤ ayaan ❤'))
}

function logPposong () {
  console.log(chalk.red('❤ pposong ❤'))
  console.log(chalk.yellow('❤ pposong ❤'))
  console.log(chalk.green('❤ pposong ❤'))
  console.log(chalk.blue('❤ pposong ❤'))
  console.log(chalk.bgRed('❤ pposong ❤'))
  console.log(chalk.bgYellow('❤ pposong ❤'))
  console.log(chalk.bgGreen('❤ pposong ❤'))
  console.log(chalk.bgBlue('❤ pposong ❤'))
}

function logDana () {
  console.log(chalk.red('❤ dana ❤'))
  console.log(chalk.yellow('❤ dana ❤'))
  console.log(chalk.green('❤ dana ❤'))
  console.log(chalk.blue('❤ dana ❤'))
  console.log(chalk.bgRed('❤ dana ❤'))
  console.log(chalk.bgYellow('❤ dana ❤'))
  console.log(chalk.bgGreen('❤ dana ❤'))
  console.log(chalk.bgBlue('❤ dana ❤'))
}

function logSaint () {
  console.log(chalk.red('❤ saint ❤'))
  console.log(chalk.yellow('❤ saint ❤'))
  console.log(chalk.green('❤ saint ❤'))
  console.log(chalk.blue('❤ saint ❤'))
  console.log(chalk.bgRed('❤ saint ❤'))
  console.log(chalk.bgYellow('❤ saint ❤'))
  console.log(chalk.bgGreen('❤ saint ❤'))
  console.log(chalk.bgBlue('❤ saint ❤'))
}

export default {
  logHyeonsu,
  logJune,
  logChoonti,
  logDante,
  logAyaan,
  logDana,
  logPposong,
  logSaint
}
